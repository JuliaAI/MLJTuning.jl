...
"""
    tuned_model = TunedModel(; model=nothing,
                             ...
                             operation=predict,
                             range=nothing,
                             selection_heuristic=OptimizePrimaryAggregatedMeasurement(),
...
- `selection_heuristic`: the rule determining how the best model is
  ...
  `OptimizePrimaryAggregatedMeasurement()`, `measure` (or the first
...
function MLJBase.clean!(tuned_model::EitherTunedModel)
    ...
    if !supports_heuristic(tuned_model.tuning, tuned_model.selection_heuristic)
        ...
        "`OptimizePrimaryAggregatedMeasurementment()`."
        tuned_model.selection_heuristic = OptimizePrimaryAggregatedMeasurement()
    end
